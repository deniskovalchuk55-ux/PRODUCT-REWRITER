...
- Не обмежуйся "очевидними" (Бренд/Колір/Вага) — переноси ВСІ, включно з вузькоспеціальними.
...
Мета — щоб товар знаходився за багатьма пошуковими запитами. Пиши грамотно; де доречно — вживай популярне слово яким реально шукають (напр. "Павербанк"), але без нагромадження синонімів і без зміни звичних назв. Довжину обирай під товар.${editNote ? `\n\nДОДАТКОВА ВКАЗІВКА ВІД КОРИСТУВАЧА (обовʼязково врахуй при генерації): ${editNote}` : ""}`;
    ...
    // 3. Фото: СПОЧАТКУ зі сторінки-джерела (Rozetka — якісні), ПОТІМ добір з Ninja
    stage = "збір фото";
    let imageCandidates = [];
    const seenImg = new Set();

    // 3.1 Фото зі сторінки-джерела (Rozetka/фід — найякісніші й точно того товару)
    try {
      // головне фото з метаданих
      if (meta.ogImage && !seenImg.has(meta.ogImage)) {
        seenImg.add(meta.ogImage);
        imageCandidates.push(meta.ogImage);
      }
      // зображення з markdown ![alt](url)
      const mdImg = /!\[[^\]]*\]\((https?:\/\/[^\s)]+)\)/gi;
      let m;
      while ((m = mdImg.exec(pageContent)) !== null) {
        const u = m[1];
        if (!seenImg.has(u) && /\.(jpg|jpeg|png|webp)/i.test(u) &&
            !u.includes("logo") && !u.includes("icon") && !u.includes("sprite") && !u.includes("placeholder")) {
          seenImg.add(u);
          imageCandidates.push(u);
        }
      }
      // прямі посилання на картинки
      const rawImg = /https?:\/\/[^\s"')]+\.(?:jpg|jpeg|png|webp)/gi;
      const found = pageContent.match(rawImg) || [];
      for (const u of found) {
        if (!seenImg.has(u) && !u.includes("logo") && !u.includes("icon") && !u.includes("sprite") && !u.includes("placeholder")) {
          seenImg.add(u);
          imageCandidates.push(u);
        }
      }
    ...
    // 3.2 Добір з Ninja (тільки ВЕЛИКІ фото від 600px — для якості)
    stage = "добір фото (Ninja)";
    try {
      const ninjaKey = process.env.NINJA_API_KEY;
      const query = (packaged.title || meta.title || "").slice(0, 120);
      if (ninjaKey && query && imageCandidates.length < 14) {
        const ninjaUrl = "https://api.openwebninja.com/realtime-image-search/search?query=" + encodeURIComponent(query) + "&limit=30";
        ...
        if (imgRes.ok) {
          ...
          // збираємо великі фото, сортуємо за розміром (найбільші перші)
          const big = [];
          for (const item of results) {
            const u = item.thumbnail_url || item.url;
            const w = item.thumbnail_width || item.width || 0;
            const h = item.thumbnail_height || item.height || 0;
            if (u && !seenImg.has(u) && w >= 600 && h >= 600) {
              big.push({ u, area: w * h });
            }
          }
          big.sort((a, b) => b.area - a.area); // найбільші перші
          for (const item of big) {
            if (!seenImg.has(item.u)) {
              seenImg.add(item.u);
              imageCandidates.push(item.u);
            }
