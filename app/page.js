...
export default function Home() {
  ...
  async function run(extra = {}) {
    ...
    try {
      const res = await fetch("/api/rewrite", {
        ...
        body: JSON.stringify({ url: url.trim(), useFirecrawl, ...extra }),
      });
  ...
  const candidates = result?.productImages || [];
  const webPhotos = result?.webImages || [];
  ...
  return (
    <main style={{ maxWidth: 760, margin: "0 auto", padding: "32px 20px", fontFamily: "system-ui, sans-serif", color: "#0f172a" }}>
      ...
      <p style={{ color: "#64748b", marginTop: 0, marginBottom: 24 }}>
        Встав посилання на товар — отримай унікальний опис, SEO та фото.
      </p>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center" }}>
        <span style={{ fontSize: 13, color: "#64748b" }}>Джерело:</span>
        <button
          onClick={() => setUseFirecrawl(true)}
          style={{
            padding: "6px 14px", borderRadius: 8, fontSize: 13, cursor: "pointer",
            border: useFirecrawl ? "2px solid #2563eb" : "1px solid #cbd5e1",
            background: useFirecrawl ? "#eff6ff" : "#fff",
            color: useFirecrawl ? "#2563eb" : "#64748b",
            fontWeight: useFirecrawl ? 600 : 400,
          }}
        >
          🛡️ Firecrawl (захищені сайти)
        </button>
        <button
      ...
      {p && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Фото ТОВАРУ зі сторінки */}
          {candidates.length > 0 && (
            <Card
              title={`📦 Фото товару зі сторінки (${candidates.length})`}
              onCopy={() => copy(candidates.join("\n"))}
            >
              <div style={{ fontSize: 13, color: "#64748b", marginBottom: 10 }}>
                Усі фото товару з джерела. Клікни щоб вибрати потрібні, або копіюй усі кнопкою згори.
              </div>
              ...
              {selectedImages.length > 0 && (
                <div style={{ marginTop: 10, fontSize: 13, color: "#475569", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span>Вибрано {selectedImages.length}</span>
                  <button onClick={() => copy(selectedImages.join("\n"))} style={{ fontSize: 12, padding: "4px 10px", borderRadius: 6, border: "1px solid #cbd5e1", background: "#fff", cursor: "pointer" }}>
                    Копіювати вибрані
                  </button>
                </div>
              )}
          ...
          {/* Фото з ІНТЕРНЕТУ (Ninja) — з посиланнями-джерелами */}
          {webPhotos.length > 0 && (
            <Card title={`🔗 Знайдені в інтернеті (${webPhotos.length})`}>
              <div style={{ fontSize: 13, color: "#64748b", marginBottom: 10 }}>
                ⚠️ Фото з інших сайтів. Перевір джерело за посиланням перед використанням — це чужі фото.
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 10 }}>
                {webPhotos.map((item, i) => (
                  <div key={i} style={{ border: "1px solid #e2e8f0", borderRadius: 8, overflow: "hidden" }}>
                    <img src={item.image} alt="" style={{ width: "100%", aspectRatio: "1", objectFit: "cover", display: "block" }} />
                    <div style={{ padding: 6, display: "flex", flexDirection: "column", gap: 4 }}>
                      <button onClick={() => copy(item.image)} style={{ fontSize: 11, padding: "3px 6px", borderRadius: 5, border: "1px solid #cbd5e1", background: "#fff", cursor: "pointer" }}>
                        Копіювати фото
                      </button>
                      {item.source && (
                        <a href={item.source} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: "#2563eb", textDecoration: "none", textAlign: "center" }}>
                          🔗 Джерело
                        </a>
                      )}
                    </div>
