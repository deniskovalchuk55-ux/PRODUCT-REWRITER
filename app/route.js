...
export async function POST(req) {
  ...
  try {
    ...
    const { url, editNote, useFirecrawl = true } = await req.json();
    ...
    // 1. Завантаження сторінки — Firecrawl (захищені сайти) АБО простий fetch (звичайні)
    let pageContent = "";
    let meta = {};

    if (useFirecrawl) {
      // === Режим Firecrawl (обходить захист, напр. Rozetka) ===
      ...
      try {
        ...
        try {
          ...
          return Response.json(
            { error: "Firecrawl повернув не JSON (можливо ліміт кредитів або захист сайту): " + fcText.slice(0, 200), stage },
            { status: 422 }
          );
        }
        ...
        if (!fcRes.ok || !fcData.success) {
          return Response.json(
            { error: "Firecrawl не зміг отримати сторінку: " + (fcData.error || fcRes.status), stage },
            { status: 422 }
          );
        }
        ...
        return Response.json({ error: "Помилка Firecrawl: " + e.message, stage }, { status: 422 });
      }
    } else {
      // === Режим простого fetch (звичайні сайти без захисту, напр. Color-It) ===
      stage = "завантаження сторінки (без Firecrawl)";
      try {
        ...
        if (!res.ok) {
          return Response.json({ error: "Сайт не відповів: " + res.status + " (спробуй з Firecrawl)", stage }, { status: 422 });
        }
        ...
        // прибираємо скрипти/стилі і теги → чистий текст
        pageContent = html
          ...
          .replace(/\s+/g, " ")
          .trim();
        // зберігаємо й сирий html для витягу фото
        meta._rawHtml = html;
      } catch (e) {
        return Response.json({ error: "Помилка завантаження: " + e.message + " (спробуй з Firecrawl)", stage }, { status: 422 });
      }
    }

    if (!pageContent || pageContent.length < 300) {
      return Response.json({ error: "Сторінка порожня або контенту замало (спробуй інший режим — з Firecrawl / без нього)", stage }, { status: 422 });
    }
...
- ОПТОВІ умови: "в ящику N штук", "мінімальне замовлення", "упаковка", кількість штук в упаковці, оптова ціна — це НЕ характеристика товару, товар описуй як ОДНУ одиницю (не як пакет/упаковку!).

🎯 ОПИСУЙ ТІЛЬКИ ОСНОВНИЙ ТОВАР — той що в головному заголовку (H1) сторінки.
- ПОВНІСТЮ ІГНОРУЙ блоки "Зверніть увагу на ці товари", "Рекомендовані", "Схожі товари", "З цим купують" та будь-які списки ІНШИХ товарів унизу/збоку сторінки. Їхні назви, коди й описи НЕ використовуй.
- Бери опис, характеристики й дані ВИКЛЮЧНО основного товару.
...
- Не нагромаджуй синонімів.${editNote ? `\n\nДОДАТКОВА ВКАЗІВКА ВІД КОРИСТУВАЧА (обовʼязково врахуй при генерації): ${editNote}` : ""}`;
    ...
    try {
      ...
      // якщо є сирий HTML (режим без Firecrawl) — витягуємо <img src="">
      if (meta._rawHtml) {
        const imgTag = /<img[^>]+src=["']([^"']+\.(?:jpg|jpeg|png|webp)[^"']*)["']/gi;
        let im;
        while ((im = imgTag.exec(meta._rawHtml)) !== null) {
          let u = im[1];
          if (u.startsWith("//")) u = "https:" + u;
          if (u.startsWith("http")) addProduct(u);
        }
      }
    } catch (e) {}
    productImages = productImages.slice(0, 20);
    delete meta._rawHtml; // прибираємо сирий html з відповіді (не потрібен клієнту)
    ...
    return Response.json({
      ...
      productImages,
      webImages,
      debug: { pageChars: pageContent.length },
    });
  } catch (err) {
